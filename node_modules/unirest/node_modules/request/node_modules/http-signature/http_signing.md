...
Copyright (c***REMOVED*** 2011 Joyent, Inc. and the persons identified as document authors.
All rights reserved.
...
requests.  RFC2617 (HTTP Authentication***REMOVED*** defines Basic and Digest authentication
mechanisms, and RFC5246 (TLS 1.2***REMOVED*** defines client-auth, both of which are widely
...
of web service requests, but is not (always***REMOVED*** ideal for machine to machine
communication, as the key acquisition steps (generally***REMOVED*** imply a fixed
...
asymmetric key (e.g., RSA***REMOVED*** or a shared symmetric key (e.g., HMAC***REMOVED***.  The scheme
...
The client is expected to send an Authorization header (as defined in RFC 2617***REMOVED***
with the following parameterization:

    credentials := "Signature" params
    params := 1#(keyId | algorithm | [headers] | [ext] | signature***REMOVED***
    ...
    plain-string   = 1*( %x20-21 / %x23-5B / %x5D-7E ***REMOVED***
...
All requests refer to the following request (body ommitted***REMOVED***:
...
The authorization header and signature would be generated as:
    ...
    Authorization: Signature keyId="rsa-key-1",algorithm="rsa-sha256",signature="Base64(RSA-SHA256(signing string***REMOVED******REMOVED***"
...
The authorization header and signature would be generated as:
    ...
    Authorization: Signature keyId="rsa-key-1",algorithm="rsa-sha256",headers="request-line date content-type content-md5",signature="Base64(RSA-SHA256(signing string***REMOVED******REMOVED***"
...
The authorization header and signature would be generated as:
    ...
    Authorization: Signature keyId="hmac-key-1",algorithm="hmac-sha1",signature="Base64(HMAC-SHA1(signing string***REMOVED******REMOVED***"
...
requests are carried over a secure transport (i.e., TLS***REMOVED***.  Sending the default
...
STRIDE threat-modeling methodology***REMOVED***.
...
is to allow 300s of clock skew (in either direction***REMOVED***.
...
* [RFC5246] The Transport Layer Security (TLS***REMOVED*** Protocol Version 1.2

## Informative References

    Name: Mark Cavage (editor***REMOVED***
    Company: Joyent, Inc.
...
The following test data uses the RSA (2048b***REMOVED*** keys, which we will refer
...
inserted for readability***REMOVED***:
