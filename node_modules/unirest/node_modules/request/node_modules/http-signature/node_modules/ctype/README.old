...
appropriate source***REMOVED***. The lower level API is presented at the end of this
document.
...
as indicated:
    ...
    * int16_t
    * int32_t
    * int64_t (returns an array where val[0] << 32 + val[1] would be the value***REMOVED***
    * uint8_t
    * uint16_t
    * uint32_t
    * uint64_t (returns an array where val[0] << 32 + val[1] would be the value***REMOVED***
    * float
    * double
    * char (returns a buffer with just that single character***REMOVED***
    * char[] (returns an object with the buffer and the number of characters read which is either the total amount requested or until the first 0***REMOVED***
...
would build a struct to represent a point (x,y***REMOVED*** as follows:
...
(as defined by int16_t***REMOVED*** to determine the Number to use for X, and then it would
...
of lseek(offset, SEEK_SET***REMOVED***. Thus, subsequent values will be read from that
...
typedef('size_t', 'uint32_t'***REMOVED***;
typedef('ssize_t', 'int32_t'***REMOVED***;
typedef('point_t', [
    { x: { type: 'int16_t' }},
    { y: { type: 'int16_t' }}
]***REMOVED***;
...
typedef('path', 'char[1024]'***REMOVED***; /* Good */
typedef('path', 'char[len]'***REMOVED***;  /* Bad! */
...
value[0] << 32 + value[1] (Note this will not work in Javascript***REMOVED***
 ...
 *
 */
function CTypeParser(conf***REMOVED***;
 ...
 *			from.
 */
CTypeParser parseCTF(data, conf***REMOVED***;
 ...
 * is the read value.
 */
Object CTypeParser.readData(<Type Definition>, buffer, offset***REMOVED***;
 ...
 */
void CTypeParser.writeData(<Type Definition>, buffer, offset***REMOVED***;
 ...
 */
void CTypeParser.prototype.typedef(name, value***REMOVED***;

Object CTypeParser.prototype.lstypes(***REMOVED***;

/*
 * Get the endian value for the current parser
 */
String CTypeParser.prototype.getEndian(***REMOVED***;
 ...
 *
 */
void CTypeParser.protoype.setEndian(String***REMOVED***;
 ...
 */
Number function toAbs64(val***REMOVED***
 ...
 *	val		An array of two 32-bit integers
 */
Number function toApprox64(val***REMOVED***
...
Number ruint8(buffer, endian, offset***REMOVED***;
Number ruint16(buffer, endian, offset***REMOVED***;
Number ruint32(buffer, endian, offset***REMOVED***;
Number[] ruint64(buffer, endian, offset***REMOVED***;

Read signed integers from a buffer:
Number rsint8(buffer, endian, offset***REMOVED***;
Number rsint16(buffer, endian, offset***REMOVED***;
Number rsint32(buffer, endian, offset***REMOVED***;
Number[] rsint64(buffer, endian, offset***REMOVED***;

Read floating point numbers from a buffer:
Number rfloat(buffer, endian, offset***REMOVED***;   /* IEEE-754 Single precision */
Number rdouble(buffer, endian, offset***REMOVED***;  /* IEEE-754 Double precision */

Write unsigned integers to a buffer:
void wuint8(Number, endian, buffer, offset***REMOVED***;
void wuint16(Number, endian, buffer, offset***REMOVED***;
void wuint32(Number, endian, buffer, offset***REMOVED***;
void wuint64(Number[], endian, buffer, offset***REMOVED***;

Write signed integers from a buffer:
void wsint8(Number, endian, buffer, offset***REMOVED***;
void wsint16(Number, endian, buffer, offset***REMOVED***;
void wsint32(Number, endian, buffer, offset***REMOVED***;
void wsint64(Number[], endian, buffer offset***REMOVED***;

Write floating point numbers from a buffer:
void wfloat(Number, buffer, endian, offset***REMOVED***;   /* IEEE-754 Single precision */
void wdouble(Number, buffer, endian, offset***REMOVED***;  /* IEEE-754 Double precision */
